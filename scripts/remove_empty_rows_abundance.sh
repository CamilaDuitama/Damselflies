...
BEGIN { FS = OFS = " " }
{
    ...
    total = NF - 1
    if (total == 0) {
        next  # Skip lines with only one column (Unitig ID)
    }
    positive = 0
    for (i = 2; i <= NF; i++) {
        split($i, values, ";")
        if (values[1] > 0) positive++
    }
...
update_progress() {
    ...
    local processed_lines=$current_progress
    echo -ne "Progress: $progress% ($processed_lines/$total_lines lines)\r"
}

# Process the file in chunks
for ((i=1; i<=total_lines; i+=lines_per_job)); do
    end=$((i + lines_per_job - 1))
    sed -n "${i},${end}p" "$input" | awk -v threshold="$threshold" "$awk_script" > "$temp_dir/chunk_$(printf "%010d" $i)" &
done
...
{
    # Print the header (first line of the first chunk)
    head -n 1 "$temp_dir"/chunk_*0000000001
    
    # Concatenate the rest of the chunks in order, skipping their headers
    for chunk in $(ls -v "$temp_dir"/chunk_*); do
        tail -n +2 "$chunk"
    done
} > "$output"
